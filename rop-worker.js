...
// NOTE: lapse-offsets.json (10.00) lists setjmp/longjmp at 0x1c8e3/0x1c93c and
// syscall_wrapper at 0x1a357 — a constant 0x260 delta from these values. Two
// RE'd datasets disagree; reconcile against the actual 10.00 binaries before
// relying on either. (Known-issue, documented in README.)
const LK_SYSCALL_WRAPPER = 0x1A5B7n;
...
function g(name) {
    switch (name) {
        ...
        case "syscall_wrapper": return W.kbase + LK_SYSCALL_WRAPPER;
    }
...
function ping(ms) {
    return new Promise((resolve, reject) => {
        ...
        W.worker.onerror = () => { clearTimeout(t); reject(new Error("worker error")); };
...
function fireSync(payload, big, spins) {
    ...
    const deadline = Date.now() + 10000;   // wall-clock backstop: ~2 min at
                                           // 6µs/rd64 is far too long to hang
    for (let i = 0; i < cap; i++) {
...
function allocReset() {
    stackBump = 0x1000n;
    // batchBuf lives at the end of the arena — it must be invalidated on reset
    // or the next alloc() carves over it (silent chain corruption).
    W.batchBuf = null;
}
...
function init(cfg) {
    // Guard: every constant in this worker is 10.00-specific (thread list
    // walk at LK_THREAD_LIST runs BEFORE the self-checks, so a wrong-FW
    // kernelBase is an unmapped userland read that kills the renderer).
    if (cfg.fw && String(cfg.fw) !== "10.00") {
        throw new Error("rop-worker: built for FW 10.00 only, got " + cfg.fw);
    }
