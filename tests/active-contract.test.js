...
assert.ok(page.includes("scheduleStagePersist"));
assert.ok(page.includes("window.__DEEPSLOP_DUMP_PROMISE = null"));
assert.ok(page.includes("window.__DEEPSLOP_ACTIVE_RUN = runToken"));
assert.ok(page.includes('q.set("run", runToken)'));
assert.ok(!page.includes("if (!wakeLockActive) initWakeLock();"));
...
assert.ok(exploit.includes("releaseTransientMemory"));
assert.ok(exploit.includes('intParam("gap", 500'));
assert.ok(exploit.includes("const RUN_TOKEN"));
assert.ok(exploit.includes("isActiveRun"));
assert.ok(exploit.includes("RCE preparing"));
assert.ok(exploit.includes("WEBKIT_RELRO_END - 0x100"));
