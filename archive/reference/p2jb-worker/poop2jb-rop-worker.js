...
function spawnWorker() {
    //note: Keep the worker asset inside the DeepSlop deployment so the adapter
    //does not depend on pooP2JB's separate hosting root.
    if (!W.worker) W.worker = new Worker("adapters/poop2jb-rop-slave.js");
    return W.worker;
}
...
function configure(cfg) {
    if (!cfg || cfg.kernelBase === undefined || cfg.kernelBase === null)
        throw new Error("rop-worker: kernel base is required");
...
window.rop_worker = {
    configure, init, survey, fire, syscall, testGetpid,
