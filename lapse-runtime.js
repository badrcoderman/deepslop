...
(function () {
    ...
    // Guard: lapse (kernel AIO double-free) only exists on FW 9.00 – 10.01.
    // This module is inert on anything else — refusing to run with a clean
    // error is safer than firing real ROP at wrong addresses.
    (function checkFwGuard() {
        const v = window.FW_VERSION;
        const ok = /^(09\.(00|03|60)|10\.(00|01))$/.test(v);
        if (!ok) {
            throw new Error("lapse-runtime: unsupported FW " + v
                + " (kernel exploit window is 9.00-10.01; webkit RCE only beyond)");
        }
    })();
